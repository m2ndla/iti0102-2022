...
def rainbows(field: str, lower=False) -> int:
    ...
    if not field:
        return 0
    if field[:7].lower() == "wobniar" or field[:7].lower() == "rainbow":
...
def create_student(name: str, grades: list, credit_points: int) -> Student:
    ...
    return Student(name, round(average, 3), credit_points)
...
def get_top_student_with_credit_points(students: list, min_credit_points: int):
    ...
    if not lst:
        ...
        sorted_lst = sorted(lst, key=lambda x: x.average_grade, reverse=True)
        return sorted_lst[0]
...
def add_result_to_student(student: Student, grades_count: int, new_grade: int, credit_points) -> Student:
    ...
    """
    current_avg = student.average_grade
    new_avg = (current_avg * grades_count + new_grade) / (grades_count + 1)
    rounded_avg = round(new_avg, 3)
    student.average_grade = rounded_avg
...
def get_ordered_students(students: list) -> list:
    ...
    """
    sorted_name = sorted(students, key=lambda x: x.name)
    sorted_grade = sorted(sorted_name, key=lambda x: x.average_grade, reverse=True)
    sorted_credit = sorted(sorted_grade, key=lambda x: x.credit_points, reverse=True)
    return sorted_credit
...
class Room:
    ...
    def add_feature(self, feature: str) -> bool:
        ...
        """
        if feature not in self.features and self.booked is False:
...
class Hotel:
    ...
    def add_room(self, room: Room) -> bool:
        ...
        for hotel_room in self.rooms:
            if self.rooms:
                if hotel_room.number == room.number:
                    return False
                else:
