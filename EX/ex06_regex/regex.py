...
def find_words_with_vowels(text: str) -> list:
    ...
    """
    ptrn = r"[AEIOUÕÜÖÄ][a-zõüöä]{1,9999}"
    result = re.findall(ptrn, text)
    return result
...
def find_sentences(text: str) -> list:
    ...
    """
    ptrn = r"[A-ZÕÜÖÄ][A-ZÕÜÖÄa-zõüöä :;,\d-]*[!?\.]{1,3}"
    result = re.findall(ptrn, text)
    return result
...
def find_phone_numbers(text: str) -> dict:
    ...
    for item in result:
        ...
        if not numberlist:
            ...
            for area_code in numberlist:
                if area_code[0] == itm[1]:
                    area_code[1].append(itm[2])
                    break
            else:
                numberlist.append([itm[1], [itm[2]]])
