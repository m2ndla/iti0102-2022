...
def read_from_file_into_list(filename: str) -> list:
    ...
    with open(f"{filename}") as file:
        for line in file.readlines():
            ...
            lst.append(obj)
    for person in lst:
        client_list.append(Client(person[0], person[1], int(person[2]), int(person[3]), int(person[4])))
    return client_list
...
def filter_by_bank(filename: str, bank: str) -> list:
    ...
    """
    bank_list = []
    lst = read_from_file_into_list(filename)
    for person in lst:
        if bank == person.bank:
            bank_list.append(person)
    return bank_list
...
def largest_earnings_per_day(filename: str) -> Optional[Client]:
    ...
    time_list = sorted(client_list, key=lambda x: x.account_age)
    earnings_list = sorted(time_list, key=Client.earnings_per_day, reverse=True)
    for person in earnings_list:
        if person.earnings_per_day() > 0:
...
def largest_loss_per_day(filename: str) -> Optional[Client]:
    ...
    client_list = read_from_file_into_list(filename)
    time_list = sorted(client_list, key=lambda x: x.account_age)
    earnings_list = sorted(time_list, key=Client.earnings_per_day)
    for person in earnings_list:
        if person.earnings_per_day() < 0:
            return earnings_list[0]
        else:
